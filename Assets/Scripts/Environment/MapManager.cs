...
public class MapManager : MonoBehaviour
{
    ...
    private Dictionary<NpcController, GameObject> activeNameTagObjects = new Dictionary<NpcController, GameObject>();
    ...
    private void RefreshNpcNameTags(FloorVisibilityManager.FloorLevel currentMapFloor)
    {
        ...
        List<NpcController> allNpcs = NpcController.AllActiveNpcs;
        ...
        foreach (NpcController npc in allNpcs)
        {
            if (npc.currentNpcFloorLevel == currentMapFloor)
            {
                ...
                if (tagInstance == null)
                {
                    Debug.LogError($"[MapManager-Refresh] INSTANTIATE FAILED for {npc.npcConfig.npcName}! Prefab: {npcNameTagPrefab.name}, Parent: {nameTagParent.name}. THIS IS A MAJOR ISSUE.");
                    continue;
                }
                tagInstance.name = $"MapTag_{npc.npcConfig.npcName}";
                ...
                if (tmpText != null)
                {
                    tmpText.text = npc.npcConfig.npcName;
                    if (string.IsNullOrEmpty(npc.npcConfig.npcName)) Debug.LogWarning($"[MapManager-Refresh] NPC {npc.gameObject.name} has empty npcName.");
    ...
    private Dictionary<NpcController, FloorVisibilityManager.FloorLevel> _lastPolledNpcFloorsForMap = new Dictionary<NpcController, FloorVisibilityManager.FloorLevel>();
    ...
    private void PollNpcFloorChangesAndRefreshTags()
    {
        ...
        Dictionary<NpcController, FloorVisibilityManager.FloorLevel> currentFrameNpcFloors = new Dictionary<NpcController, FloorVisibilityManager.FloorLevel>();
        foreach (NpcController npc in allNpcs)
        {
