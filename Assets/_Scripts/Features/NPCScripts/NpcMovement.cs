...
public class NpcMovement : MonoBehaviour
{
    ...
    /// <summary>
    /// Commands the NPC to continuously turn to face a target until told to stop.
    /// </summary>
    public void StartTrackingTarget(Transform target, float turnSpeed = 5f)
    {
        if (!this.enabled) return;
        _trackingTarget = target;
        _trackingTurnSpeed = turnSpeed;
        if (_turningCoroutine != null) StopCoroutine(_turningCoroutine); // Stop any one-off turns.
    }

    /// <summary>
    /// Commands the NPC to stop tracking its target.
    /// </summary>
    ...
    {
        if (_trackingTarget == null) return; // Not tracking anything, do nothing.

        Vector3 direction = _trackingTarget.position - transform.position;
        direction.y = 0; // Keep rotation horizontal

        if (direction.sqrMagnitude < 0.001f) return;

        Quaternion targetRotation = Quaternion.LookRotation(direction.normalized);
        transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * _trackingTurnSpeed);
    }
